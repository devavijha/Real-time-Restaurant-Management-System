...
import { X, ShoppingCart, Clock, Check, ChevronLeft, Menu, Star, PlusCircle, MinusCircle } from 'lucide-react';
...
const CustomerView = ({ tableId, onLogout }: CustomerViewProps) => {
  ...
  const { getOrdersByTable, createOrder } = useOrder();
  
  const [selectedCategory, setSelectedCategory] = useState<string | null>(null);
  const [cartItems, setCartItems] = useState<(OrderItem & { menuItem: MenuItem })[]>([]);
  const [isCartOpen, setIsCartOpen] = useState(false);
  const [activeOrders, setActiveOrders] = useState<ReturnType<typeof getOrdersByTable>>([]);
  const [completedOrders, setCompletedOrders] = useState<ReturnType<typeof getOrdersByTable>>([]);
  ...
  const updateOrders = () => {
    ...
    setActiveOrders(orders.filter(order => !['completed', 'cancelled'].includes(order.status)));
    setCompletedOrders(orders.filter(order => ['completed', 'cancelled'].includes(order.status)));
  };
  
  const handleAddToCart = (menuItem: MenuItem, quantity: number = 1, modifiers?: any[]) => {
  ...
  return (
    <div className="min-h-screen bg-gray-50 flex flex-col">
      ...
      <header className="bg-white shadow-sm sticky top-0 z-30">
        <div className="container mx-auto px-4 py-4 flex items-center justify-between">
          ...
          <div className="flex items-center space-x-4">
            ...
            <div className={`fixed inset-0 bg-gray-800 bg-opacity-75 z-40 transition-opacity duration-300 ${
                ...
                <div className="mt-12 space-y-4">
                  <button 
                    ...
                    className={`w-full text-left py-2 px-4 rounded-lg ${
                      viewMode === 'menu' ? 'bg-blue-50 text-blue-600' : 'text-gray-700'
                    }`}
                  >
                    Menu
                  </button>
                  ...
                  <button 
                    ...
                    className={`w-full text-left py-2 px-4 rounded-lg ${
                      viewMode === 'orders' ? 'bg-blue-50 text-blue-600' : 'text-gray-700'
                    }`}
                  >
                    Your Orders
          ...
          <div className="space-y-8">
                      ...
                      {order.feedback && (
                        <div className="mt-4 border-t border-gray-100 pt-4">
                            ...
                            <div className="flex">
                              {[...Array(5)].map((_, i) => (
                                <Star
                                  key={i}
                                  size={16}
                                  className={i < order.feedback!.rating ? 'fill-amber-400 text-amber-400' : 'text-gray-300'}
                                />
                              ))}
                            </div>
                          ...
                          {order.feedback.comment && (
                            <p className="text-sm text-gray-600 mt-1">{order.feedback.comment}</p>
                          )}
                        </div>
                      )}
                    </div>
            ...
            {activeOrders.length === 0 && completedOrders.length === 0 && (
              <div className="text-center py-10">
                ...
                <p className="text-gray-500 mb-6">Browse the menu and place your first order</p>
                <button
                  onClick={() => setViewMode('menu')}
      ...
      {/* Cart Sidebar */}
      <div className={`fixed inset-y-0 right-0 w-full md:w-96 bg-white shadow-xl z-50 transform transition-transform duration-300 ${
              ...
              <div className="space-y-4">
                {cartItems.map((item, index) => (
                  <CartItem
                    ...
                    onUpdateQuantity={(quantity) => handleUpdateCartItemQuantity(index, quantity)}
                  />
                ))}
              </div>
            )}
